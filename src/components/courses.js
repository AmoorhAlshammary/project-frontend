...
function Courses({token ,userId}) {
 ...
 const [RateArr, setRateArr] = useState([0,0,0,0,0])
  ...
  useEffect(() => {
    const getData = async () => {
        console.log(token);
        const respone = await axios.get("https://amirah-backend.herokuapp.com/hotel" , {headers: { authorization: `Bearer ${token}` }})
  ...
  const AddCourse = async () => {
    const respone = await axios.post("https://amirah-backend.herokuapp.com/hotel" , {
      name: name , 
  ...
  const deleteCourse = async (id) =>{
    const respone = await axios.delete("https://amirah-backend.herokuapp.com/hotel/" + id , 
    {headers: { authorization: `Bearer ${token}` }})
    setData(respone.data)
  }

  const deleteAll = async () => {
    const respone = await axios.delete("https://amirah-backend.herokuapp.com/deleteAll")
    setData(respone.data)
  }

  const updateData = async (id) => {
    // console.log(id);
    const respone = await axios.put("https://amirah-backend.herokuapp.com/hotel/" , {
      idold : id , 
      name : nameUp , 
